...
# --- User specified a target model or key ---
if [ $# -gt 0 ]; then
    TARGET="$1"
    ...
    if [[ "$TARGET" == *".gguf"* ]]; then
        ...
        # If they passed a raw string, handle whether it
    ...
    if podman ps -a --format "{{.Names}}" | \
       grep -q "^${CONTAINER_NAME}$"; then
        ...
        echo "ℹ️ No container named '$CONTAINER_NAME' is \
currently active or suspended."
    fi

# --- No arguments, clean up everything matching the prefix ---
else
    echo "Searching for containers matching prefix '${CONTAINER_PREFIX}-'..."
    
    # Get list of container names matching the prefix
    MATCHING_CONTAINERS=$(podman ps -a --format "{{.Names}}" | \
        grep "^${CONTAINER_PREFIX}-" || true)
    
    if [ -z "$MATCHING_CONTAINERS" ]; then
        echo "✅ No containers found matching prefix \
'${CONTAINER_PREFIX}-'. Everything is clean."
        exit 0
    fi
